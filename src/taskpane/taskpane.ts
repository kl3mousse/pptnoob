...
Office.onReady(() => {
  ...
  document
    ...
    .addEventListener("input", resetIconResults);
  document
    .getElementById("iconWeight")!
    .addEventListener("change", resetIconResults);
  document
    .getElementById("showMoreIcons")!
    .addEventListener("click", showMoreIcons);
  loadIcons();
});
...
async function loadIcons(): Promise<void> {
  try {
    ...
    if (!response.ok)
      throw new Error("Could not load the Phosphor icon index.");
    const index = (await response.json()) as IconIndex;
    icons = Object.entries(index).reduce<IconDefinition[]>(
      (allIcons, [weight, filenames]) =>
        allIcons.concat(
    ...
    setStatus(
      `${iconsPerWeight.toLocaleString()} Phosphor icons available in ${weightCount} weights.`,
    );
  } catch (error: unknown) {
...
function getMatchingIcons(): IconDefinition[] {
  const query = (
    document.getElementById("iconSearch") as HTMLInputElement
  ).value
    .trim()
    .toLowerCase();
  const weight = (document.getElementById("iconWeight") as HTMLSelectElement)
    .value;
  return icons.filter(
    (icon) => icon.weight === weight && icon.slug.includes(query),
  );
}
...
function renderIcons(): void {
  ...
  grid.replaceChildren(
    ...matches.slice(0, visibleIconCount).map(createIconButton),
  );
...
async function insertIcon(icon: IconDefinition): Promise<void> {
  try {
    ...
    const color = (document.getElementById("iconColor") as HTMLInputElement)
      .value;
...
async function openContainingFolder(
  ...
  try {
    ...
    if (url.protocol !== "https:" && url.protocol !== "http:") {
      throw new Error(
        "Finder cannot be opened from an Office web add-in. This command supports SharePoint and OneDrive files.",
      );
    }
...
(
  globalThis as typeof globalThis & {
    openContainingFolder: typeof openContainingFolder;
  }
).openContainingFolder = openContainingFolder;

async function setSelectedShapeMargins(
  points: number,
  commandEvent: Office.AddinCommands.Event,
): Promise<void> {
  try {
...
async function layoutSelectedShapes(
  command: ShapeLayoutCommand,
  commandEvent: Office.AddinCommands.Event,
): Promise<void> {
  try {
    await PowerPoint.run(async (context) => {
      ...
      const right = Math.max(
        ...shapes.items.map((shape) => shape.left + shape.width),
      );
      const top = Math.min(...shapes.items.map((shape) => shape.top));
      const bottom = Math.max(
        ...shapes.items.map((shape) => shape.top + shape.height),
      );
      ...
      if (command === "alignLeft") {
        ...
        shapes.items.forEach((shape) => {
          shape.top = middle - shape.height / 2;
        });
        ...
        shapes.items.length >= 3 &&
        command === "distributeHorizontally"
      ) {
        const orderedShapes = [...shapes.items].sort(
          (first, second) => first.left - second.left,
        );
        const totalWidth = orderedShapes.reduce(
          (sum, shape) => sum + shape.width,
          0,
        );
        ...
        const orderedShapes = [...shapes.items].sort(
          (first, second) => first.top - second.top,
        );
        const totalHeight = orderedShapes.reduce(
          (sum, shape) => sum + shape.height,
          0,
        );
...
function bringToFront(commandEvent: Office.AddinCommands.Event): Promise<void> {
  return setSelectedShapesZOrder(
